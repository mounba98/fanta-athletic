...
- **Bollino "Nessuna squadra" che compariva a chi la squadra ce l'ha** (causa trovata registrando cosa leggeva): `auth-guard.js` **riscriveva il nome utente a ogni pagina**; la lettura del profilo fatta nello stesso istante vedeva solo quei campi (senza `team_index`). Corretto alla fonte (`auth-guard.js?v=20260925d`, vecchio in `archive/auth-guard-pre-D113.js`): se nome utente e varianti sono già salvati uguali non scrive nulla (meno scritture sul database a ogni pagina); in più `navbar-profile-icon.js?v=20260925b` legge il profilo dal percorso vero (radice) e dal server. Verificato 5 volte su 5 (prima falliva a caso): niente bollino, titoli interi. `app-init.js?v=20260925k`.

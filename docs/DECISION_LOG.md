...
5. **Stato multileghe**: l'amico dice che **"multilega funziona ed era
   ...
   §5): `docs/storico-originali/STATO_SITO.md` riportava la migrazione delle query nelle pagine
...
Su richiesta dell'utente scritto `docs/firebase-storico/FIRESTORE_RULES_CONTEST.md`:
...
bozza regole `docs/firebase-storico/FIRESTORE_RULES_CONTEST.md` aggiornata (valida
...
- `archive/resources-orfani/contest-reminder.js` (Home): avviso "Voto aperto, non hai
...
intero; riscritto `docs/firebase-storico/FIRESTORE_RULES_CONTEST.md` con **4 modifiche
...
`docs/firebase-storico/FIRESTORE_RULES_CONTEST.md` aggiornato. Nulla era ancora stato
...
- **Fatto**: `archive/resources-orfani/contest-reminder.js` riscritto come card fissa (stato: voto aperto / già votato / prossimo voto + punti delle due fazioni); voce "Mini-gioco" nel menu mobile e nella barra desktop; `classifiche.html#fazioni` apre direttamente la scheda + pulsante "Vota la prossima partita". Verificato in Home (mobile) e Classifiche, nessun errore.
...
**Documenti riallineati**: `CLAUDE.md` (sezione regole Firestore non più vera; fase attiva), `docs/MAPPA_PAGINE.md` (mancavano `admin-contest.html`, `admin-fazioni.html`; 76 pagine), `docs/firebase-storico/FIRESTORE_RULES_CONTEST.md` (diceva "non applicate"), `docs/PUNTI_APERTI.md`.
...
- **`sw.js` riscritto** (vecchio in `archive/versioni-precedenti/sw-pre-D099.js`, cache `fanta-athletic-v2026092501`): JS/CSS con etichetta `?v=` e librerie Firebase locali → **prima il magazzino** (la prima volta chiede al server, non alla cache del browser); HTML e file senza etichetta → prima la rete, con copia di riserva offline; quando arriva un'etichetta nuova la versione vecchia dello stesso file viene tolta. Precarica Firebase all'installazione. Toglie dalla lista file che non esistono più. Ignora le richieste verso altri domini (Firebase/Google).
...
**Fatto (vecchie versioni in `archive/versioni-precedenti/h2h-pre-D100.js`, `archive/versioni-precedenti/calendario-pre-D100.html`):**
...
- **Pubblicato** (25/09/2026 22:43): commit `31188d5` su GitHub + sito (`firebase deploy --only hosting`, 192 file). Verificato sul sito vero: nuova Home, Archivio Athletic, motore statistiche, selettore, finestra allenatore online; `.git`, backup del database e documenti → 404.

## D112 — Competizioni = tornei tra le squadre (3); Curva vs Piana è un "gioco collaterale" (2026-09-25)
- **Correzione dell'utente**: le competizioni sono **Campionato, Scontri diretti, Coppa** (tornei tra le squadre del fanta); Curva vs Piana mette in sfida le tifoserie con i voti dei singoli → **gioco collaterale** della stagione. Glossario aggiornato.
- **Scoperto**: in app non esiste un selettore di competizione; oggi sono sparse (Campionato = Classifiche › Squadre; Scontri = scheda nascosta finché spenti + Calendario + pagina scontri; Coppa = sezione del Calendario + Pannello, spenta, senza classifica). **Proposta**: sezione "Competizioni 2026/27" dentro la tendina 🏆 (solo quelle accese). In attesa della scelta dell'utente: competizione che "guida" Classifiche/Calendario (consigliato) oppure semplici scorciatoie.

## D113 — Prove dal telefono: tendina, swipe, freccia "indietro", bollino "Nessuna squadra" (2026-09-25)
**Segnalazioni dell'utente dal telefono**: (1) con la tendina 🏆 aperta, toccando fuori si attivava il collegamento sotto; (2) lo swipe da sinistra a volte apre il menu, a volte torna indietro — "togliamo il più possibile gli aspetti da web app"; (3) da Home › Statistiche "sono rimasto intrappolato": serve sempre una freccia per uscire. **Decisione**: il selettore di competizione (D112) va fatto "da app" (la competizione scelta guida Classifiche e Calendario) — da costruire.
**Fatto:**
- **Tendina 🏆** (`league-selector.js?v=20260925j`): velo trasparente dietro la tendina (solo su telefono, dove la tendina è spostata nella pagina): un tocco fuori la chiude e **non** attiva ciò che c'è sotto. Verificato: tocco su "Classifica" con tendina aperta → tendina chiusa, pagina invariata.
- **Swipe** (`edge-swipe-menu.js?v=20260925`, vecchio in `archive/versioni-precedenti/edge-swipe-menu-pre-D113.js`; ora su tutte le 28 pagine col menu, aggiunto a `live.html`): riconosce lo swipe partendo fino a 40 px dal bordo, apre il menu dopo 28 px (prima 60), blocca il gesto del browser già al primo tocco nei primi 16 px (mai sopra pulsanti/link, es. ☰). Simulato: apre da 8 px e da 35 px, non apre dal centro né con uno scorrimento verticale. **Limite onesto**: il gesto "indietro" di sistema (Safari su iPhone, navigazione a gesti Android) ha la precedenza e non si può spegnere da un sito; nell'app installata sulla Home (iPhone) il gesto di Safari non esiste. Da riprovare sul telefono vero.
- **Freccia ‹ "indietro"** (`mobile-menu.js?v=20260925f`): su telefono, in tutte le pagine secondarie (tutte tranne le 5 della barra in basso) accanto a ☰: torna alla pagina precedente dell'app, altrimenti alla Home. Spazi dell'intestazione stretti e titolo a 18 px su queste pagine. Verificato: Statistiche → freccia → Home; titoli interi.
- **Bollino "Nessuna squadra" che compariva a chi la squadra ce l'ha** (causa trovata registrando cosa leggeva): `auth-guard.js` **riscriveva il nome utente a ogni pagina**; la lettura del profilo fatta nello stesso istante vedeva solo quei campi (senza `team_index`). Corretto alla fonte (`auth-guard.js?v=20260925d`, vecchio in `archive/versioni-precedenti/auth-guard-pre-D113.js`): se nome utente e varianti sono già salvati uguali non scrive nulla (meno scritture sul database a ogni pagina); in più `navbar-profile-icon.js?v=20260925b` legge il profilo dal percorso vero (radice) e dal server. Verificato 5 volte su 5 (prima falliva a caso): niente bollino, titoli interi. `app-init.js?v=20260925k`.
- **Selettore delle competizioni "da app"** (decisione utente: la competizione scelta guida le pagine, "perché il senso interattivo della Home non si perda"):
  - nuovo `resources/competitions.js`: competizioni accese nella stagione (Campionato se modalità ≠ "scontri"; Scontri diretti se "scontri"/"entrambe"; Coppa se `leagues/{lega}.settings.cupSystem.enabled`) e competizione scelta, ricordata sul dispositivo per lega (`fa_competizione_{lega}`), evento `competition-changed`;
  - tendina 🏆: nuova sezione **"Competizioni 2026/27"** tra leghe e pulsanti (caricata all'apertura; `competitions.js` caricato al bisogno); scegliendo una competizione si va alla sua pagina: Campionato → Classifiche (Squadre), Scontri → Classifiche (scheda Scontri), Coppa → Calendario (vista Coppa);
  - `classifiche.html` si apre sulla competizione scelta (o `#h2h`/`#squadre`), **dopo** aver caricato i dati: se gli scontri sono spenti resta su Squadre; `calendario.html` apre la vista Coppa con `#coppa`.
  - **Difetto trovato prima di pubblicare**: una vecchia regola fissa la tendina a livello 10000 "con forza", quindi il velo (10040) le stava sopra e **le voci non si potevano toccare** → velo a 9990. Verificato: voce della tendina toccabile; pagina, ☰ e barra in basso coperti dal velo (un tocco lì chiude e basta).
...
## D115 — La guida per gli admin diventa una pagina dell'app (2026-09-26)
**Richiesta dell'utente**: vedendo il PDF aperto nel pannello ("sembrava una pagina vera"), farla davvero in app, così gli admin hanno la guida a portata di mano; peso sulle prestazioni trascurabile. **Perché è una buona idea**: una pagina a sé si carica solo quando la si apre (zero effetto sulle altre pagine), e diventa l'**unica fonte**: il PDF si ricava da lei invece di tenere due copie da allineare.
**Fatto (in locale):**
- Nuova `guida-admin.html` (titolo "Guida admin"): tutto il testo della v2, indice con link ai capitoli, tema chiaro/scuro dell'app, su telefono le tabelle diventano schede impilate (con "Quando:" per la terza colonna), pulsante "Salva come PDF / stampa" (in stampa sparisce tutto tranne la guida, un capitolo per pagina). Solo per admin, con lo stesso controllo delle altre pagine admin (non admin → "Accesso riservato"; non loggato → login). Menu, freccia ‹, swipe, barra in basso e "← Pannello admin" come le altre pagine admin.
- `admin.html`: chip **📖 Guida** (bordo oro) in testa alla barra dei blocchi.
- `scripts/guida-admin-pdf.py` (cartella `scripts/` esclusa dal sito): rigenera il PDF da `guida-admin.html` con Chrome invisibile su file locale, nel solito posto fuori da Git. Rigenerato: 12 pagine, uguale alla v2. Il vecchio sorgente HTML è in `03-documenti-admin/versioni-precedenti/`.
- **Da sapere**: il testo della pagina, come ogni file del sito, è tecnicamente leggibile da chi conosce l'indirizzo (il blocco "solo admin" nasconde il contenuto, non lo cifra). Controllato: contiene solo istruzioni d'uso, nessun dato personale o segreto.
- Verificato: telefono (scuro e chiaro), computer 1280 px, indice e link ai capitoli, nessuna barra orizzontale, nessun errore in console.
- **Regola nuova**: quando cambia una funzione admin, aggiornare anche la guida (scritto in `CLAUDE.md`).
**Prossimo passo**: commit+push e pubblicazione del sito, su conferma dell'utente.

## D116 — Rifiniture (bordo oro, "New"), Novità dell'app, regola "tutto collegato" (2026-09-26)
**Decisioni dell'utente:**
1. Il bordo oro sull'Archivio (e sul tasto Guida) sembrava "selezionato", ma era un segno "novità" voluto (D109). Scelta: **bordo oro tenue su tutte le caselle**, non solo in Home, dove serve coerenza; il tasto **Guida** resta con l'oro pieno per staccarsi dagli altri. L'app sembra ancora "un pelo grezza": rifiniture estetiche da continuare.
2. Funzioni nuove segnate con un'**etichetta "New" a segnalibro** in alto a destra.
3. **Novità dell'app** (come nei giochi da telefono): comunicati sugli aggiornamenti per gli utenti e, contrassegnati, solo per gli admin; **li scrive Claude** con il contesto per chi legge. Fattibile subito → fatto. **Notifiche push** rimandate (priorità dopo).
4. **Regola generale dell'app**: tutte le funzioni "parallele" (guida admin, Novità, ecc.) vanno aggiornate a ogni modifica che le riguarda **senza che l'utente lo chieda**, con un collegamento esplicito perché non si tralasci nulla, controllando anche il lavoro già fatto.
**Fatto (in locale):**
- `resources/ui-kit.css` (nuovo, condiviso): `--tile-border` (oro tenue, versione chiara e scura), etichetta `.fa-new` a segnalibro rosso, stili delle Novità. Applicato a Home (`.hd-card`, `.hd-tile`, `.hd-mini`; le caselle di stato verde/ambra/rosso tengono il loro colore, quelle "Solo admin" restano tratteggiate grigie), pannello admin (`.tile`; chip Guida con oro pieno), Novità, casella Novità in Notifiche. Tolto il vecchio segno "novità" (`.hd-new`); le etichette verdi "Nuovo" del pannello diventano "New" a segnalibro. Le etichette "New" hanno una scadenza (`data-new-until`, oggi 15/11/2026) e spariscono da sole (`resources/news.js`).
- **Novità**: `resources/news.json` (comunicati, pubblicati col sito: niente database né regole nuove), `resources/news.js` (lette/non lette ricordate sul dispositivo; su un dispositivo nuovo quelle con più di 30 giorni contano come lette), `novita.html` (elenco; "Solo admin" visibili solo agli admin), casella **Novità dell'app** in Home con il numero delle non lette, **finestra all'apertura** della Home con l'ultima non letta (una per apertura, mai sopra la finestra obbligatoria tifoseria/allenatore; pulsanti sempre visibili), voce **Novità** nel menu ☰ (`mobile-menu.js?v=20260926` su 30 pagine), collegamento da Notifiche (che resta per le notifiche personali della lega). Prime due novità scritte: "L'app si rinnova per la stagione 2026/27" (tutti) e "Per gli admin: il pannello nuovo e la guida" (solo admin). **Da sapere**: come la guida, il testo delle novità "solo admin" è nascosto ai giocatori ma non segreto; contiene solo istruzioni d'uso.
- **Collegamento esplicito** (regola 4): `docs/CHECKLIST_AGGIORNAMENTO.md` (se cambi X → aggiorna Y) + `scripts/check-coerenza.py`, da lanciare prima di ogni commit: controlla "Allineato a" di tutti i documenti, che guida (`meta guida-allineata`) e Novità (`allineato`) siano state valutate per l'ultima decisione, che ogni casella del pannello sia citata nella guida, che ogni pagina sia nella mappa, le etichette `?v=` (file condivisi cambiati senza etichetta nuova, etichette diverse tra pagine), le etichette "New" senza scadenza o scadute, il PDF più vecchio della pagina. Regola in cima a `CLAUDE.md` e nella memoria di Claude.
- **Controllo del lavoro passato** (primo giro dello script): mancavano nella mappa `archivio-athletic.html` (D109, dimenticata) e `novita.html`; `MAPPA_PAGINE.md` era fermo a D113; `cache-buster.html` usava un'etichetta vecchia di `sheet.css`. Sistemati. Tutte le caselle del pannello risultano già nella guida. Guida aggiornata con Novità ed etichette "New" (capitolo 2 + domanda frequente), PDF rigenerato.
- Titolo del pannello: "Pannello admin" → **"Pannello"** (con la freccia ‹ veniva tagliato coi puntini).
- Verificato su telefono: finestra Novità (prima quella per tutti, poi alla riapertura quella admin), "Ho capito" / "Tutte le novità", casella con contatore che sparisce dopo la lettura, pagina Novità, caselle col bordo oro, etichette "New" su Archivio e pannello, voce di menu; nessun errore.
**Prossimo passo**: commit+push e pubblicazione (guida in app D115 + D116), su conferma dell'utente.

## D117 — Novità dentro la Bacheca, etichetta "New" più piccola e per 2 settimane, regola anche in claude-config (2026-09-26)
**Decisioni dell'utente** (dopo aver visto D116):
1. Bordo oro tenue: approvato; valutare poi se estenderlo ad altre parti.
2. Etichetta "New": **più piccola e più a destra** (non deve coprire l'icona della casella), **per 2 settimane dalla pubblicazione** dell'aggiornamento, poi via.
3. **Le Novità stanno in Bacheca** ("è tutto community come idea"): non più una pagina e una voce di menu a sé.
4. La regola "tutto collegato" va scritta anche nelle regole di lavoro generali (`~/Claude/claude-config/WORKING_PRACTICES.md`). Ricontrollare ancora che sia tutto a posto.
**Fatto:**
- `resources/ui-kit.css?v=20260926b`: etichetta "New" più piccola (7 px, 20 px di larghezza) e a 6 px dal bordo destro; stili dell'elenco Novità e delle schede della Bacheca spostati qui.
- `bacheca.html`: due schede in cima, **Messaggi** (il solito) e **Novità app** (con il numero delle non lette); `bacheca.html#novita` apre direttamente le Novità. L'elenco è disegnato da `FANews.renderList` (`resources/news.js?v=20260926b`), che le segna come lette.
- Tolti `novita.html` (mai pubblicata) e la voce "Novità" del menu ☰; casella in Home, finestra all'apertura e collegamento in Notifiche portano a Bacheca › Novità app. Testi di guida e novità aggiornati di conseguenza.
- Etichette "New" fino al **10/10/2026** (2 settimane da una pubblicazione prevista il 26/09; se slitta, spostare la data). Lo script avvisa se una scadenza supera le 3 settimane.
- Regola generale aggiunta a `WORKING_PRACTICES.md` (claude-config).
- Etichetta "New", su richiesta dell'utente ("schiacciata verticalmente, proporzionala"): segnalibro proporzionato **22×27 px**, testo 7 px (`ui-kit.css?v=20260926c`). Non copre l'icona della casella.
- **Ricontrollo**: `scripts/check-coerenza.py` → tutto coerente; cercati a mano residui (`novita.html`, vecchio `.hd-new`, etichette "Nuovo"): nessuno; guida e novità rilette e corrette (dove si trovano le Novità, durata "New"); PDF rigenerato; provato su telefono: Bacheca con le due schede e contatore, `#novita` dalla casella in Home, finestra all'apertura, etichette "New", pannello; console pulita in una scheda nuova (nessun errore, avviso, permesso negato o file mancante) su Bacheca e Pannello.
**Prossimo passo**: commit+push del progetto e del file di regole `claude-config`, poi pubblicazione del sito, su conferma dell'utente.
- **Pubblicato** (26/09/2026): commit `d7a5c6d` su GitHub + sito (`firebase deploy --only hosting`); regola generale in `claude-config` (commit `6b53dde`, già inviato dall'auto-push). Verificato sul sito vero: `news.json` (allineato D117), `news.js`, `ui-kit.css`, `guida-admin.html`, schede della Bacheca e casella Novità in Home online; `scripts/`, `docs/`, `.git` e la vecchia `novita.html` → 404. `CLAUDE.md` preparato per la prossima sessione (blocco "Parti da qui" e piano generale: non diceva più il vero sulla stagione).

## D118 — Riordino della cartella di lavoro (2026-09-26)
**Richiesta dell'utente**: controllare che la cartella sia pulita e ben organizzata, rinominando/riorganizzando dove serve; poi commit, push e pubblicazione.
**Principio**: solo spostamenti ("spostare mobili"), nessun effetto sul funzionamento. Le 80 pagine `.html` restano nella cartella principale: senza un "navigatore interno" i collegamenti assumono che stiano lì (D021).
**Fatto:**
- **12 documenti storici di Mocci** dalla cartella principale a `docs/storico-originali/` (+ `LEGGIMI.md`): `STATO_SITO`, `ARCHITETTURA_SISTEMA_V2` e `_COMPLETA`, `ANALISI_MIGRAZIONE_MULTILEGHE`, `MULTILEGA-PLAN`, `CONSIGLIO_MIGRAZIONE`, `GUIDA_TEST_MIGRAZIONE`, `STRUTTURA_FIRESTORE_POST_MIGRAZIONE`, `BACKUP_FIRESTORE`, `REQUISITI_STORE_E_ADMIN`, `ADSENSE_SETUP`, `GITHUB_SETUP`. Contenuto invariato.
- `backup-firestore.sh/.bat` (backup cloud di Mocci) → `scripts/`.
- `docs/FIRESTORE_RULES_CONTEST.md` (regole già applicate, confluite in `firestore.rules`) e `resources/cors.json` (impostazione del bucket Storage, non usata da nessuna pagina) → `docs/firebase-storico/` (LEGGIMI aggiornato). Il messaggio di errore di `admin-contest.html` ora rimanda a `firestore.rules`.
- **9 "versioni precedenti"** sparse in `archive/` → `archive/versioni-precedenti/`; nuovo `archive/LEGGIMI.md` che spiega ogni sottocartella.
- **7 file di `resources/` non caricati da nessuna pagina** (verificato con ricerca su tutto il progetto) → `archive/resources-orfani/`: `classifiche-preview.js`, `contest-reminder.js`, `matchday-summary.js`, `mobile-dashboard.js` (vecchia Home, sostituita in D109), `admin-navbar.html` (mai collegata), `coaches_/players_y3GQ05….json` (dati di prova di una lega vecchia).
- **`backups-firestore/`** (copia dei backup del database dentro il progetto: dati personali) verificata **identica byte per byte** a quella in `~/Claude/fanta-athletic-backups/02-database/` → spostata nel **Cestino** (recuperabile). Il server locale (`.claude/no-cache-server.py`) ora salva i backup nuovi direttamente lì, fuori dal progetto. Esclusioni in `.gitignore`/`firebase.json` lasciate come rete di sicurezza.
- Riferimenti aggiornati in `CLAUDE.md`, `README.md`, `docs/` (anche i percorsi citati nelle voci vecchie del diario, perché restino validi), `index.html` (solo un commento).
